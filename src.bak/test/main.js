...
async function fileHandle(fullpath) {
    ...
    if (fs.statSync(fullpath).size > 1024 * 1024 * 30) {
...
function archiveHandle(fileJSON) {
    ...
    let savePath = path.join(fileJSON.archivePath, `${fileJSON.modify}-[${fileJSON.lables.join(",")}]-${fileJSON.md5}-${fileJSON.archiveName}`)
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        try {
            ...
            if (stats.isDirectory()) {
                ...
                basename = basename.replaceAll("已归档 - ", "")

                fs.renameSync(fullpath, path.join(dirname, basename))
                // let file = await fileHandle(fullpath)
